...
namespace Nano35.Instance.Api.Requests.CreateClient
{
    public class CreateClientUseCase : UseCaseEndPointNodeBase<ICreateClientRequestContract, ICreateClientSuccessResultContract>
    {
        ...
        public override async Task<UseCaseResponse<ICreateClientSuccessResultContract>> Ask(ICreateClientRequestContract input)
        {
            input.UserId = _auth.CurrentUserId;
            input.Phone = PhoneConverter.RuPhoneConverter(input.Phone);
            return await new MasstransitUseCaseRequest<ICreateClientRequestContract, ICreateClientSuccessResultContract>(_bus, input)
                .GetResponse();
        }
    }
}