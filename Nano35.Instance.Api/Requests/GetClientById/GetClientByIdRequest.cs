...
namespace Nano35.Instance.Api.Requests.GetClientById
{
        ...
        IPipelineNode<IGetClientByIdRequestContract, IGetClientByIdResultContract>
    {
        ...
        public async Task<IGetClientByIdResultContract> Ask(IGetClientByIdRequestContract input)
        {
            ...
            var client = _bus.CreateRequestClient<IGetClientByIdRequestContract>(TimeSpan.FromSeconds(10));
            
            // Receive response of processor magic
            var response = await client
                .GetResponse<IGetClientByIdSuccessResultContract, IGetClientByIdErrorResultContract>(input);
            
            // Checking response status
            if (response.Is(out Response<IGetClientByIdSuccessResultContract> successResponse))
                return successResponse.Message;
            
            if (response.Is(out Response<IGetClientByIdErrorResultContract> errorResponse))
