...
namespace Nano35.Instance.Api.Requests.UpdateWorkersComment
{
            ...
            IUpdateWorkersCommentResultContract>
    {
            ...
            IUpdateWorkersCommentRequestContract input)
        {
            ...
            var client = _bus.CreateRequestClient<IUpdateWorkersCommentRequestContract>(TimeSpan.FromSeconds(10));
            
            // Receive response of processor magic
            var response = await client
                .GetResponse<IUpdateWorkersCommentSuccessResultContract, IUpdateWorkersCommentErrorResultContract>(input);
            
            // Checking response status
            if (response.Is(out Response<IUpdateWorkersCommentSuccessResultContract> successResponse))
                return successResponse.Message;
            
            if (response.Is(out Response<IUpdateWorkersCommentErrorResultContract> errorResponse))
