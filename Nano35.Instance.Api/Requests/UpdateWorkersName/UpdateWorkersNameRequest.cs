...
namespace Nano35.Instance.Api.Requests.UpdateWorkersName
{
            ...
            IUpdateWorkersNameResultContract>
    {
            ...
            IUpdateWorkersNameRequestContract input)
        {
            ...
            var client = _bus.CreateRequestClient<IUpdateWorkersNameRequestContract>(TimeSpan.FromSeconds(10));
            
            // Receive response of processor magic
            var response = await client
                .GetResponse<IUpdateWorkersNameSuccessResultContract, IUpdateWorkersNameErrorResultContract>(input);
            
            // Checking response status
            if (response.Is(out Response<IUpdateWorkersNameSuccessResultContract> successResponse))
                return successResponse.Message;
            
            if (response.Is(out Response<IUpdateWorkersNameErrorResultContract> errorResponse))
